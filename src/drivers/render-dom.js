...
let fromEvent = require(`../../node_modules/@cycle/dom/lib/fromevent`)
let VDOM = {
  h: require(`../../node_modules/@cycle/dom/lib/virtual-hyperscript`),
  diff: require(`../../node_modules/@cycle/dom/node_modules/virtual-dom/diff`),
  patch: require(`../../node_modules/@cycle/dom/node_modules/virtual-dom/patch`),
  parse: typeof window !== `undefined` ? require(`../../node_modules/@cycle/dom/node_modules/vdom-parser`) : () => {},
}
let {replaceCustomElementsWithSomething, makeCustomElementsRegistry} =
  require(`../../node_modules/@cycle/dom/lib/custom-elements`)
let {transposeVTree} = require(`../../node_modules/@cycle/dom/lib/transposition`)
let matchesSelector
// Try-catch to prevent unnecessary import of DOM-specifics in Node.js env:
try {
  matchesSelector = require(`../../node_modules/@cycle/dom/node_modules/matches-selector`)
} catch (err) {
  matchesSelector = () => {}
}
...
function makeElementSelector(rootElem$) {
  return function select(selector) {
    ...
    return {
      ...
      select: makeElementSelectorNested(element$),
    }
  }
}

function makeElementSelectorNested(element$) {
  return selector => {
    return {
      events: event => element$
        .filter(elements => !!elements[0])
        .map(elements => elements[0].querySelector(selector))
        .flatMap(els => Rx.Observable.fromEvent(els, event)).shareReplay(1),
    }
  }
}
